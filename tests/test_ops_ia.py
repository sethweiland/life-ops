"""Home inbox, peer tabs, and Folders IA for the life-ops shell."""
...
def _ops_app() -> Flask:
    """Home/projects/real X + Grok Bot; meme routes are href stubs only."""
    app = Flask(
        __name__,
    ...
    app.secret_key = "test"
    spend = Blueprint("spend", "spend")
    spend.add_url_rule("/spend/", "index", lambda **_kwargs: "")
    app.register_blueprint(spend)
...
class OpsIaTests(unittest.TestCase):
    ...
    def test_memes_pipeline_is_not_served(self):
        """modules.memes may stay on as a Folders flag; this app does not serve the pipeline."""
    ...
    def test_old_meme_pipeline_urls_are_gone(self):
        for path in (
            "/generate/",
            "/gallery/",
            "/templates/",
