...
Token writers default to the tenant ``defaults.usage_project`` unless
``USAGE_PROJECT`` or ``MEME_PROJECT`` is set. Untagged historical xAI
rows still resolve to ``memes`` so existing ledgers do not jump buckets.
"""
