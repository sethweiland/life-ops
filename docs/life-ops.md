...
(`sethweiland/life-ops`). A friend should be able to fork, plug in their
projects, and run their own instance. It is not a multi-tenant SaaS.

Live `ops.sethweiland.com` still runs from `sethweiland/memes` (Fly app
`meme-ops`) until cutover. This extract is **config-driven** and must not
import the meme pipeline.
...
| `memes` | `/memes/` (404 here) | Flag only. Hides/no-ops folder links when off. The pipeline stays in `sethweiland/memes`. |
...
See `.env.example`. AWS / Bitwarden backends are documented names only.
...
The human reviews X drafts on `/x/`. Meme daily candidates stay in
`sethweiland/memes`.
...
- A sixth primitive
- Switching production Fly (`meme-ops`) or Cloudflare in this extract
- Copying the meme generation / Imgflip / video / RAG pipeline
- Rewriting the X, Spend ledger, or Grok Bot catalog UIs
